extends CanvasLayer
## 对白气球（复制自 Dialogue Manager 示例气球）。改外观在编辑器里编辑本场景即可。
...
## 切换对白后的输入宽容时间：这段时间内忽略推进输入，避免旧对话的按键把新对话瞬间跳完。
var _input_grace_remaining: float = 0.0
...
## 设置输入宽容时间：这段时间内忽略推进输入（用于打断换对话后防误跳）。
func set_input_grace(seconds: float) -> void:
	_input_grace_remaining = maxf(_input_grace_remaining, seconds)


func _process(_delta: float) -> void:
	_input_grace_remaining = maxf(_input_grace_remaining - _delta, 0.0)
	if is_instance_valid(dialogue_line):
		progress.visible = not dialogue_label.is_typing and dialogue_line.responses.size() == 0 and not dialogue_line.has_tag("voice")


func _unhandled_input(_event: InputEvent) -> void:
	# 打断换对话后的输入宽容期：忽略推进输入，防止旧对话按键把新对话瞬间跳完
	if _input_grace_remaining > 0.0:
		return
