...
const 动画表: Array[Dictionary] = [
	# idle 2026-09-22 改成**单帧**（作者："idle 你不做动画得了，就用单帧"——
	# 之前那版呼吸动画里 88 一直在抬头）。单帧 + 循环 = 完全静止。
	# 这一帧同时是**尺寸基准**（它的内容高 247 决定 `scale`）与 intro 末帧的对接姿势。
	{"名": "idle",       "帧": 1,  "循环": true},
	{"名": "walk",       "帧": 16, "循环": true},
	# intro 36 → **72**（2026-09-22）：作者要 3 秒的登场演出（坐姿弹琴 → 起身 →
	# 黑洞漩涡把钢琴卷走 → 转身站定），36 帧 = 1.5 秒装不下。
	{"名": "intro",      "帧": 72, "循环": false},
	...
	# hurt 2026-09-22 **删除**：作者观察"似乎没用到"——核实属实。
	# Boss 的 `take_damage()` 只调 `_闪白()`（白色闪光 + 受击音），从不播 `hurt` 动画；
	# 搜到的 `anim.play("hurt")` 是蝙蝠/骷髅自己的动画，与 88 无关。
	{"名": "knocked",    "帧": 12, "循环": false},
	# weakened 2026-09-22 拆成两段（作者："做一个从站立、双腿支撑不住跪下，单膝跪地，
	# 然后呼吸这样的动画，随后在后半段单膝跪地呼吸循环播放"）：
	#   weakened_fall = 站姿 → 双腿撑不住 → 单膝跪地（**不循环**）
	#   weakened      = 单膝跪地的呼吸循环（**循环**）
	# 为什么要两段：`AnimatedSprite2D` **不能只循环一段动画的一部分** ——
	# 单条 32 帧循环会把"跪下"那一段也一起重播。两段同源（同一条视频相邻两窗），接得上。
	{"名": "weakened_fall", "帧": 12, "循环": false},
	{"名": "weakened",   "帧": 20, "循环": true},
]
