...
const 动画表: Array[Dictionary] = [
	...
	# ⚠️ 爪击三段的**后摇**（命中帧之后的帧数）2026-09-21 减半：
	#    16 帧 (0.67s) → 8 帧 (0.33s)。作者原话："有问题的是后摇，所以三连击每段
	#    间隔特别大。后摇减半。" 前摇与命中帧序号不动（10 / 10 / 16）。
	{"名": "claw_1",     "帧": 18, "循环": false},   # 10 前摇悬停出手 + 8 后摇
	{"名": "claw_2",     "帧": 18, "循环": false},
	{"名": "claw_3",     "帧": 24, "循环": false},   # 16 + 8 后摇
