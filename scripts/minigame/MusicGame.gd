...
## 同一首曲搬到别的关卡不该再发一次二段跳。
##
## ⚠️⚠️ 下面那个 setter 不是多余的：编辑器保存场景时会给"没改动过"的导出写一行
##     `"奖励能力" = null`（本项目反复出现的老毛病），于是它真的变成 null ——
##     `奖励能力 == ""` 会漏过去，接着拼 `"has_" + null` 当场报错。
##     场景里没填这个值的实例（第一个 MusicGame 就是）一通关就会踩到，所以统一收口成空串。
@export var 奖励能力: String = "":
	set(value):
		奖励能力 = value if value is String else ""
