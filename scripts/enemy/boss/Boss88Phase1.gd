...
@export_group("判定盒几何")
## 判定盒中心相对身体的**前伸距离**。判定盒前缘 = 本值 + 形状半宽(75) = **150**，
## 正好等于 近身盒半径 —— 玩家站在"近身"带边缘时爪击刚好够得着。
## 判定盒**后缘落在身体中心**（不是浮在身前），因为爪击带前压：近距离时 88 会冲过玩家
## 身边，盒子如果整个浮在前面就会越过玩家打空（自检里"三段各命中一次"曾因此只中 2 段）。
## ⚠️ 改体型/换贴图时这里要跟着调 —— 以前它是硬编码的 56，不随体型更新。
@export var 判定盒前伸: float = 75.0

@export_group("位移距离")
## 出手时向前压的距离（像素）。**按距离驱动、不按帧数** ——
## 顿帧会把 time_scale 压到 0，等帧数会让位移归零，等距离只是把动作拉长。
## 作者 2026-09-21 实机反馈："距离太短" → 距离翻倍（60→120 / 40→80）；
## 速度一度被翻了两番，但作者随后确认"三连击本身速度没问题，有问题的是后摇"，
## 于是速度回退到只翻一番这一档。**真正让连段显得拖的是后摇帧数**（见 88_frames.tres）：
## 每段命中帧之后原本还有 16 帧 = 0.67 秒，现已减半到 8 帧 = 0.33 秒。
@export var 爪击前突距离: float = 120.0
@export var 爪击三前突距离: float = 80.0
## 前突刺的冲程。设计稿写 225px，实机手感要 5 倍（= 417px）才够得着中距带远端
@export var 突刺距离: float = 417.0
@export var 爪击前突速度: float = 1040.0
@export var 爪击三前突速度: float = 720.0
@export var 突刺速度: float = 1250.0
## 冲刺保险丝：最多等这么多物理帧（防"距离永远走不到"时卡死协程）
@export var 冲刺最长帧: int = 600
## 冲刺连续这么多物理帧几乎没有位移 → 认定顶到了墙/场地边界，提前收招。
## 没有它的话：88 贴到 活动左/右 再出招，`_滑行` 每帧把位置夹回去，
## 位移永远到不了目标值 → 会空转满 冲刺最长帧（600 帧 = **10 秒僵直**）。
@export_range(2, 20, 1) var 冲刺卡住帧: int = 3
## 位置采样比施加位移晚约这么多物理帧，冲刺提前这么多帧收手，抵消过冲。
## 实测（2080px/s，单帧 34.7px）：0 帧过冲 +69px、1 帧 +53px、2 帧 **+19px**、3 帧 -16px。
## 逐帧量化做不到精确命中，取 2 —— **宁可可过一点也不要短**，攻击够得着比省距离重要。
@export_range(0, 6, 1) var 冲刺提前帧: int = 2
## 闪现落点离玩家多远：`突刺距离` 的比例（0.5 = 半个前突刺的距离）。
## 用比例而不是绝对值 —— 改 突刺距离 时落点自动跟着走。
@export_range(0.2, 1.5, 0.05) var 闪现落点比例: float = 0.5
## 落点离玩家近于这个距离就退回另一侧（贴场地边缘时夹取会把落点挤到玩家身上）
@export var 闪现最小落点间距: float = 60.0
...
## 落点：出现在玩家的**另一侧** —— 88 在玩家左边就落到玩家右边，反之亦然。
## 落地后由 `_出闪现背刺` 调 `_face_player()` 转身，朝玩家方向前突刺。
##
## ⚠️ 作者原话："不要同侧出现，那闪现的没有意义"。返工前这里取的是
##    `signf(88.x - 玩家.x)` = **88 现在所在的那一侧**，于是落点始终在
##    "玩家 → 88" 那一侧 —— 和同侧没区别（变量名还叫 `背后`，名字与实现相反）。
##
## 受战斗区镜头锁死约束：落点夹在 `活动左~活动右` 内。夹完若离玩家太近
## （玩家贴着场地边缘、想要的那一侧没空间），退回原来那一侧 ——
## 瞬移到玩家身上比同侧出现更糟。
func _闪现落点() -> Vector2:
	if player == null:
		return global_position
	var 玩家x := player.global_position.x
	# 玩家的另一侧：88 在左 → 取右；88 在右 → 取左
	var 侧 := -signf(global_position.x - 玩家x)
	if 侧 == 0.0:
		侧 = 1.0
	var 距离 := 突刺距离 * 闪现落点比例
	var x := clampf(玩家x + 侧 * 距离, 活动左, 活动右)
	if absf(x - 玩家x) < 闪现最小落点间距:
		x = clampf(玩家x - 侧 * 距离, 活动左, 活动右)
...
##
## ⚠️ **但"等位移"有个反面**：`_滑行()` 每帧把 `global_position.x` 夹回
##    `活动左~活动右`。88 贴到场地边界再出招时，位移永远到不了目标值 →
##    循环会空转满 `冲刺最长帧`（600 帧 = 10 秒僵直）。所以加卡住检测。
##    顿帧期间（time_scale = 0）位移天然为 0，**不能算卡住**，否则上面那条就废了。
## 尾巴帧：冲程走完后盒子再多留几帧，覆盖"玩家刚好贴在判定边缘"的情况。
##
## ⚠️ **位置采样比施加位移晚约 2 个物理帧**（协程在 `physics_frame` 信号里恢复，
##    那时本帧的 `move_and_slide()` 还没跑）。所以要在"还差约 2 帧行程"时就收手，
##    否则实际会冲过头约 2 帧 —— 1040px/s 时是 36px，2080px/s 时就是 69px，
##    比 120px 的目标一半还多。`冲刺提前帧` 就是补这个的。
func _冲刺(距离: float, 速度: float, token: int, 尾巴帧: int = 0) -> bool:
	...
	var 卡住 := 0
	var 剩余 := 冲刺最长帧
	var 单帧 := 速度 / float(maxi(Engine.physics_ticks_per_second, 1))
	var 目标 := maxf(距离 - 单帧 * float(冲刺提前帧), 0.0)
	while 剩余 > 0:
		await get_tree().physics_frame
		剩余 -= 1
		if not _攻击有效(token):
		...
		if not _结算过:
			_结算过 = _结算命中()
		var 现在 := global_position.x
		if Engine.time_scale > 0.0 and absf(现在 - 上次) < 0.5:
			卡住 += 1
			if 卡住 >= 冲刺卡住帧:
				break
		else:
