...
## 作者 2026-09-21 实机反馈"三连击太拖沓、距离太短"→ 距离与速度各翻倍。
## 两者同时翻倍 ⇒ **单段时长不变**（120/1040 = 60/520 = 0.115s），
## 所以前摇预警与玩家反应时间都不变，只是每段多推 2 倍地面、读起来利索。
...
##
## ⚠️ **实际位移会比 `距离` 多出约 2 帧的行程**（1040px/s 时约 36px）——
##    位移是逐物理帧量化的，停下时已经过了头；这不是失控（有卡住检测与帧数保险丝）。
##    自检里对距离的断言因此用区间（下限 0.9×、上限 1.6×）而不是等号。
