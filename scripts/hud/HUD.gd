...
func _connect_boss() -> void:
	...
	if boss and boss.has_signal("boss_battle_started"):
		boss.boss_battle_started.connect(_on_boss_battle_started)
	if boss and boss.has_signal("health_changed") and bool(boss.get("battle_started")):
...
func _on_boss_battle_started() -> void:
	var boss := get_tree().get_first_node_in_group("boss")
	if boss == null or not boss.has_signal("health_changed"):
		return
	if not boss.is_connected("health_changed", _on_boss_health_changed):
		boss.health_changed.connect(_on_boss_health_changed)
	_on_boss_health_changed(boss.hp, boss.max_hp)
