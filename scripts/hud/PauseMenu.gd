...
func _ready() -> void:
	...
	_settings_panel = preload("res://scenes/ui/GameSettingsPanel.tscn").instantiate()
...
func _build_pause_panel() -> void:
	...
	_cheat_double_jump = _make_cheat_toggle("踏音而行", _on_cheat_double_jump, GameState.has_double_jump)
	_cheat_magic_dash = _make_cheat_toggle("碎影颤音", _on_cheat_magic_dash, GameState.has_magic_dash)
	_cheat_magic_climb = _make_cheat_toggle("凌空音阶", _on_cheat_magic_climb, GameState.has_magic_climb)
	_cheat_magic_flight = _make_cheat_toggle("华彩终章", _on_cheat_magic_flight, GameState.has_magic_flight)
	...
	_cheat_heal_melody = _make_cheat_toggle("悠远的旋律", _on_cheat_heal_melody, GameState.has_heal_melody)
	...
	box.add_child(_make_button("游戏设置", _on_settings))
...
func _refresh_cheat_toggles() -> void:
	...
	_cheat_heal_melody.button_pressed = GameState.has_heal_melody
	_cheat_invincible.button_pressed = _is_invincible_enabled()
...
func _on_cheat_heal_melody(v: bool) -> void:
	GameState.has_heal_melody = v


func _on_cheat_invincible(v: bool) -> void:
	var player := get_tree().get_first_node_in_group("player") as CharacterBody2D
	if player and player.has_method("set_cheat_invincible"):
		player.set_cheat_invincible(v)


func _is_invincible_enabled() -> bool:
	var player := get_tree().get_first_node_in_group("player") as CharacterBody2D
	return player != null and bool(player.get("cheat_invincible"))
...
func _on_main_menu() -> void:
	get_tree().paused = false
	_settings_panel.visible = false
	AudioManager.stop_all_music(true)
	GameState.save_game()
